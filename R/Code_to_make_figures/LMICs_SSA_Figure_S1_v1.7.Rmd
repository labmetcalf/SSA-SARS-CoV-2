...
1. SSA testing data: `manual_africa_cdc_data_20200924.csv`
...
#1
df.testing <- read_csv(here("data/Data/manual_africa_cdc_data_20200924.csv"))
#2
...
names(df.plot) <- c("COUNTRY_NAME", "COUNTRY_CODE", "cases", "deaths", "tests", "popn")
...
f.popn_specific_ifr <- function(demo.age.str){
...
df.exps <- df.plot %>% group_by(COUNTRY_CODE) %>%
  mutate(exp_death_per1inf = f.country.demo.age.str(as.character(COUNTRY_CODE)) %>% f.popn_specific_ifr) %>% 
  ungroup() %>% 
  ...
  melt(id = c("COUNTRY_NAME", "COUNTRY_CODE", 
              "cases", "deaths", "tests", 
              "popn", 
              "cases_p100k", "deaths_p100k", "tests_p100k", 
              "positivity", "cfr",
              "rank_for_plotting",
              "exp_death_per1inf", "exp_inf"))

#Setting colors
exp_inf_cols <- c("exp_perc_inf.100R" = "#880e4f", 
...
exp_inf_labs <- c("100%", 
                  ...
                  "25%", 
                  "10%")
names(exp_inf_labs) <- names(exp_inf_cols)

df.exps %>% filter(exp_inf > 0) %>%
  ...
  scale_y_log10(labels = scales::number_format(accuracy = 0.01), breaks = c(0.01, 0.10, 1.00, 5.00, 10.00, 50.00, 100.00)) +
  scale_color_manual(values = exp_inf_cols, labels = exp_inf_labs,
