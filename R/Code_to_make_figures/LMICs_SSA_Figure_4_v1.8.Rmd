---
title: "SARS-CoV-2 and LMICs: Risk Tiering in Africa v1.8"
author: "Benny Rice"
date: "6/30/2020"
...
3. Metapopulation model output data: `dataBenMetapop.csv`
...
6. Climate model output data: `TSAfrica.csv`
...
#3
filename.df.metapop <- "dataBenMetapop.csv"
df.metapop <- read.csv(file.path(mainDir, filename.df.metapop), stringsAsFactors = FALSE)
#4
filename.air <- "Princeton_Uni_AFRICA_JobId1590035.csv"
df.air <- read.csv(file.path(mainDir, filename.air), stringsAsFactors = FALSE)
#5
filename.hop <- "hopkins_time_series_covid19_confirmed_global.csv"
df.hop <- read.csv(file.path(mainDir, filename.hop), stringsAsFactors = FALSE)
#6
filename.df.clim <- "TSAfrica.csv"
df.clim <- read.csv(file.path(mainDir, filename.df.clim), stringsAsFactors = FALSE)
```
...
**Figure 4 | Part F | Climate model output: Time courses for 58 SSA cities**
...
df.clim <- melt(df.clim, id.vars = c("Country", "City", "Lat", "Lon", "NearestLandBased")) %>% 
  group_by(City) %>% 
  mutate(time = 1:length(City), week = 1:length(City)/7, week.r = round(1:length(City)/7)) %>% ungroup()

df.clim <- df.clim[df.clim$variable %in% c(paste0("V", rep(65:429))), ]

df.max_min.clim <- df.clim %>% group_by(City) %>% 
  summarize(peak_day = time[value == max(value)], 
            max_I_N = max(value), 
            peak_week = time[value == max(value)]/7) %>% ungroup()

#Max: Windhoek
#Min: Bissau
#Mid: Antananarivo

df.clim <- df.clim %>% mutate(color_group = case_when(
  City == "Windhoek" ~ 1,
  City == "Antananarivo" ~ 2,
  City == "Lome" ~ 3,
  TRUE ~  0
...
df.clim$color_group <- factor(df.clim$color_group, levels = unique(df.clim$color_group))

ggplot(df.clim, aes(x = week, y = value, color = color_group, group = City, alpha = color_group)) +
  scale_color_manual(values = c("gray", "#ad1457", "#ad1457", "#ad1457")) + 
  geom_line(size = 0.7) +
  scale_alpha_manual(values = c(0.2, 0.7, 0.7, 0.7)) +
  scale_x_continuous(breaks = seq(from = 5, to = 60, by = 5)) +
  xlab("Time (Weeks after March 1, 2020)") +
