...
7. Climate model output data (for supplement where R0min and R0max are varied): 
  - `TSAfrica1_2.5_20201022.csv`
  - `TSAfrica1.5_5__20201022.csv`
  - `TSAfrica1.1_2.5_20201029.csv`
  - `TSAfrica1.2_2.5_20201029.csv`
  - `TSAfrica1.3_2.5_20201029.csv`
  - `TSAfrica1.4_2.5_20201029.csv`
...
df.clim.rmin1_1 <- read_csv(here("data/Data/TSAfrica1.1_2.5_20201029.csv"))
df.clim.rmin1_2 <- read_csv(here("data/Data/TSAfrica1.2_2.5_20201029.csv"))
df.clim.rmin1_3 <- read_csv(here("data/Data/TSAfrica1.3_2.5_20201029.csv"))
df.clim.rmin1_4 <- read_csv(here("data/Data/TSAfrica1.4_2.5_20201029.csv"))
```
...
df.clim.rmin1_1  <- df.clim.rmin1_1  %>% select(-X1) %>% select(-(V6:V76)) %>% select(-(V441:V465)) %>% 
  drop_na %>% 
  melt(id.vars = c("Country", "City", "Lat", "Lon", "NearestLandBased")) %>% 
  filter(value > 0) %>%
  group_by(City) %>% 
  mutate(time = 1:length(City), week = 1:length(City)/7, week.r = round(1:length(City)/7)) %>% ungroup() %>%
  mutate(color_group = case_when(City == "Windhoek" ~ 1, City == "Antananarivo" ~ 2, City == "Lome" ~ 3, TRUE ~  0))
df.clim.rmin1_2  <- df.clim.rmin1_2  %>% select(-X1) %>% select(-(V6:V76)) %>% select(-(V441:V465)) %>% 
  drop_na %>% 
  melt(id.vars = c("Country", "City", "Lat", "Lon", "NearestLandBased")) %>% 
  filter(value > 0) %>%
  group_by(City) %>% 
  mutate(time = 1:length(City), week = 1:length(City)/7, week.r = round(1:length(City)/7)) %>% ungroup() %>%
  mutate(color_group = case_when(City == "Windhoek" ~ 1, City == "Antananarivo" ~ 2, City == "Lome" ~ 3, TRUE ~  0))
df.clim.rmin1_3  <- df.clim.rmin1_3  %>% select(-X1) %>% select(-(V6:V76)) %>% select(-(V441:V465)) %>% 
  drop_na %>% 
  melt(id.vars = c("Country", "City", "Lat", "Lon", "NearestLandBased")) %>% 
  filter(value > 0) %>%
  group_by(City) %>% 
  mutate(time = 1:length(City), week = 1:length(City)/7, week.r = round(1:length(City)/7)) %>% ungroup() %>%
  mutate(color_group = case_when(City == "Windhoek" ~ 1, City == "Antananarivo" ~ 2, City == "Lome" ~ 3, TRUE ~  0))
df.clim.rmin1_4  <- df.clim.rmin1_4  %>% select(-X1) %>% select(-(V6:V76)) %>% select(-(V441:V465)) %>% 
  drop_na %>% 
  melt(id.vars = c("Country", "City", "Lat", "Lon", "NearestLandBased")) %>% 
  filter(value > 0) %>%
  group_by(City) %>% 
...
df.max_min.rmin1_1 <- df.clim.rmin1_1 %>% group_by(City) %>% 
  summarize(peak_week = week.r[value == max(value)], max_I_N = max(value)) %>% ungroup()
df.max_min.rmin1_2 <- df.clim.rmin1_2 %>% group_by(City) %>% 
  summarize(peak_week = week.r[value == max(value)], max_I_N = max(value)) %>% ungroup()
df.max_min.rmin1_3 <- df.clim.rmin1_3 %>% group_by(City) %>% 
  summarize(peak_week = week.r[value == max(value)], max_I_N = max(value)) %>% ungroup()
df.max_min.rmin1_4 <- df.clim.rmin1_4 %>% group_by(City) %>% 
  summarize(peak_week = week.r[value == max(value)], max_I_N = max(value)) %>% ungroup()

df.clim.rmin1_1$color_group  <- factor(df.clim.rmin1_1$color_group, levels = unique(df.clim.rmin1_1$color_group))
df.clim.rmin1_2$color_group  <- factor(df.clim.rmin1_2$color_group, levels = unique(df.clim.rmin1_2$color_group))
df.clim.rmin1_3$color_group  <- factor(df.clim.rmin1_3$color_group, levels = unique(df.clim.rmin1_3$color_group))
df.clim.rmin1_4$color_group  <- factor(df.clim.rmin1_4$color_group, levels = unique(df.clim.rmin1_4$color_group))


p4_1 <- df.clim.rmin1_1 %>% filter(week.r < 61) %>%
  ggplot(aes(x = week, y = value, color = color_group, group = City, alpha = color_group)) +
  scale_color_manual(values = c("gray", "#ff9100", "#cddc39", "#5677fc")) + 
  geom_line(size = 0.7) +
  scale_alpha_manual(values = c(0.2, 0.9, 0.9, 0.9)) +
  scale_x_continuous(breaks = seq(from = 0, to = 60, by = 10)) +
  scale_y_continuous(labels = scales::number_format(accuracy = 0.01), 
                breaks = c(0.00, 0.05, 0.10),
                limits = c(0.00, 0.14)) +
  xlab("Time (weeks since start of outbreak)") +
  ylab("I/N") +
  ggtitle("R0min = 1.1, R0max = 2.5") +
  theme(panel.background = element_blank(),
        legend.position  = "none")

p4_2 <- df.clim.rmin1_2 %>% filter(week.r < 61) %>%
  ggplot(aes(x = week, y = value, color = color_group, group = City, alpha = color_group)) +
  scale_color_manual(values = c("gray", "#ff9100", "#cddc39", "#5677fc")) + 
  geom_line(size = 0.7) +
  scale_alpha_manual(values = c(0.2, 0.9, 0.9, 0.9)) +
  ...
  xlab("Time (weeks since start of outbreak)") +
  ylab("I/N") +
  ggtitle("R0min = 1.2, R0max = 2.5") +
  theme(panel.background = element_blank(),
        legend.position  = "none")

p4_3 <- df.clim.rmin1_3 %>% filter(week.r < 61) %>%
  ggplot(aes(x = week, y = value, color = color_group, group = City, alpha = color_group)) +
  scale_color_manual(values = c("gray", "#ff9100", "#cddc39", "#5677fc")) + 
  geom_line(size = 0.7) +
  scale_alpha_manual(values = c(0.2, 0.9, 0.9, 0.9)) +
  ...
  xlab("Time (weeks since start of outbreak)") +
  ylab("I/N") +
  ggtitle("R0min = 1.3, R0max = 2.5") +
  theme(panel.background = element_blank(),
        legend.position  = "none")

p4_4 <- df.clim.rmin1_4 %>% filter(week.r < 61) %>%
  ggplot(aes(x = week, y = value, color = color_group, group = City, alpha = color_group)) +
  scale_color_manual(values = c("gray", "#ff9100", "#cddc39", "#5677fc")) + 
  geom_line(size = 0.7) +
  scale_alpha_manual(values = c(0.2, 0.9, 0.9, 0.9)) +
  ...
  xlab("Time (weeks since start of outbreak)") +
  ylab("I/N") +
  ggtitle("R0min = 1.4, R0max = 2.5") +
  theme(panel.background = element_blank(),
        legend.position  = "none")

grid.arrange(p4_1, p4_2, p4_3, p4_4, nrow = 4)
